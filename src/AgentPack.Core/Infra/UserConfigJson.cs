...
public static class UserConfigJson
{
    ...
    public static JsonObject LoadObject(string path)
    {
        ...
        try
        {
            var node = JsonNode.Parse(text, documentOptions: Lenient);
            return node as JsonObject ?? throw new AgentPackException(
                $"{path} is not valid JSON object content: the root must be an object.",
        ...
        catch (JsonException ex)
        {
            throw new AgentPackException(
                $"{path} is not valid JSON object content: {ex.Message}",
                "Fix the file (or move it aside) and rerun; agentpack never overwrites a config it cannot parse.",
                ExitCodes.ValidationFailed);
        }
